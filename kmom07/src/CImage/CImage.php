...
private function imageInfo($imagePath, $verbose){
    $imageInformation = list($width, $height, $type, $attr) = getimagesize($imagePath);
    !empty($imageInformation) or $this->errorMessage("The file doesn't seem to be an image.");
    $mime = $imageInformation['mime'];

    if($verbose){
    $filesize = filesize($imagePath);
    $this->verbose("Image file: {$imagePath}");
    $this->verbose("Image information: " . print_r($imageInformation, true));
...
private function createCacheName($imagePath, $saveAs, $newWidth, $newHeight, $quality, $cropToFit, $sharpen, $verbose){
    ...
    $dirName = preg_replace('/\//', '-', dirname($imagePath));
...
private function readImageFromFile($imagePath, $fileExtension, $verbose){
    if($verbose)
...
private function processImage($imagePath, $fileExtension, $cropToFit, $width, $height, $newWidth, $newHeight, $sharpen, $verbose){
    $image = $this->readImageFromFile($imagePath, $fileExtension, $verbose);
...
public function getProcessedImage($src = null, $saveAs = null, $quality = 60, $ignoreCache = null, $newWidth = null, $newHeight = null, $cropToFit = null, $sharpen = null, $verbose = false){
    ...
    $imageInformation = $this->imageInfo($imagePath, $verbose);
    ...
    $cacheFileName = $this->createCacheName($imagePath, $saveAs, $newWidth, $newHeight, $quality, $cropToFit, $sharpen, $verbose);
    if($ignoreCache || !$this->isCached($cacheFileName, filemtime($imagePath), $verbose)){
    $image = $this->processImage($imagePath, pathinfo($imagePath)['extension'], $cropToFit, $width, $height, $newWidth, $newHeight, $sharpen, $verbose);
